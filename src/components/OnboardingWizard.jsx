...
import { 
  ...
  Briefcase, Target, Layers, Compass, CheckSquare, Zap, Lock, Sparkles
} from 'lucide-react';
...
export default function OnboardingWizard({ profile, setProfile, onFinish }) {
  ...
  return (
    <div id="onboarding-wizard-container" className="w-full bg-white rounded-xl border border-gray-200 p-6 md:p-8 shadow-sm relative flex flex-col justify-between min-h-[540px] transition-all duration-300">
      
      {/* 1. LANDING STATE (First thing visitors see - NO step counter or progress bar) */}
      {isLanding ? (
        <div id="landing-state-container" className="space-y-6 py-1 animate-fade-up">
          
          {/* Trust Strip */}
          <div id="landing-trust-strip" className="flex items-center space-x-2 text-xs font-semibold text-gray-500 bg-gray-50 px-3 py-1.5 rounded-md border border-gray-200 w-fit">
            <Shield className="w-3.5 h-3.5 text-[#0A3D62]" />
            <span>Trusted by leaders across Egypt 🇪🇬 · UAE 🇦🇪 · Saudi Arabia 🇸🇦</span>
          </div>

          {/* Hero Headline & Subtext */}
          <div className="space-y-2">
            <h1 className="text-3xl md:text-4xl lg:text-[40px] font-bold text-[#0A3D62] font-heading leading-tight tracking-tight">
              Build Trust. Unlock Opportunities.
            </h1>

            <p className="text-base text-gray-600 max-w-xl leading-relaxed">
              Whether you're an agent, architect, contractor, lawyer, marketer, or consultant—build a verified professional identity that helps decision-makers choose you with confidence.
            </p>
          </div>

          {/* Live Interactive Hero Inputs (Types live into preview card) */}
          <div id="landing-hero-inputs" className="bg-gray-50 p-4 rounded-xl border border-gray-200 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold uppercase tracking-wider text-gray-500">
                Try it live: Enter your details below
              </span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label htmlFor="landing-input-name" className="block text-xs font-medium text-gray-600 mb-1">
                  Your Full Name
                </label>
                <input
                  ...
                  className="w-full bg-white border border-gray-300 rounded-lg px-3.5 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#0A3D62]/20 focus:border-[#0A3D62] transition-all"
                />
              </div>

              <div>
                <label htmlFor="landing-input-title" className="block text-xs font-medium text-gray-600 mb-1">
                  Title or Specialty
                </label>
                <input
                  id="landing-input-title"
                  type="text"
                  value={profile.title || ''}
          ...
          {/* Animated Benefits List (Staggered reveal & checkmark scale) */}
          <div id="landing-benefits-section" className="space-y-3 pt-1">
            ...
            <div id="benefits-grid" className="grid grid-cols-1 sm:grid-cols-2 gap-y-2.5 gap-x-4 text-sm text-gray-700">
              {benefitsList.map((benefit, i) => {
                const isHighlighted = !hasUserTyped && highlightedIndex === i;
                return (
                  <div
                    key={i}
                    style={{ animationDelay: `${i * 110}ms` }}
                    className={`flex items-center space-x-2.5 p-2 rounded-lg transition-all duration-300 animate-fade-up ${
                      isHighlighted ? 'bg-[#0A3D62]/05 font-medium text-[#0A3D62]' : ''
                    }`}
                  >
                    <div
                      style={{ animationDelay: `${i * 110 + 150}ms` }}
                      className="animate-check-scale shrink-0"
                    >
                      <Check className={`w-4 h-4 ${isHighlighted ? 'text-[#0A3D62]' : 'text-[#0A3D62]/80'}`} />
                    </div>
                    <span>{benefit}</span>
                  </div>
                );
        ...
        /* 2. BUILDER STATE (After clicking CTA - Shows STEP 1 OF 12 & Form Fields) */
        <div id="builder-state-container" className="space-y-6 flex-1 flex flex-col justify-between animate-fade-up">
          ...
          {currentStep === 1 && (
            <div id="step-1-container" className="space-y-5">
              <div>
          ...
          {/* STEP 6: PHOTO */}
          {currentStep === 6 && (
            <div id="step-6-container" className="space-y-5 text-center">
              <div>
                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
              ...
              <div className="flex justify-center my-4">
                <div className="w-24 h-24 rounded-full p-1 bg-gradient-to-tr from-[#0A3D62] to-gray-400 shadow-md relative">
                  <img
                    src={profile.photo || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=500&auto=format&fit=crop&q=80"}
                    alt="Profile Avatar"
          ...
          {currentStep === 7 && (
            <div id="step-7-container" className="space-y-4">
              <div>
                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
                  STEP 7 • VERIFICATION CENTER
                </span>
          ...
          {/* STEP 8: TRACK RECORD */}
          {currentStep === 8 && (
            <div id="step-8-container" className="space-y-5">
              <div>
                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
                  STEP 8 • TRACK RECORD
                </span>
          ...
          {/* STEP 9: PROOF CENTER */}
          {currentStep === 9 && (
            <div id="step-9-container" className="space-y-4">
              <div>
                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
                  STEP 9 • PROOF CENTER
                </span>
          ...
          {currentStep === 10 && (
            <div id="step-10-container" className="space-y-5 text-center py-4">
              <div>
                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
          ...
          {/* STEP 11: SUMMARY */}
          {currentStep === 11 && (
            <div id="step-11-container" className="space-y-4">
              <div>
                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
                  STEP 11 • SUMMARY REVIEW
                </span>
                <h2 className="text-2xl font-bold text-gray-900 font-heading mt-1">
                  Review your Decision Profile
                </h2>
                <p className="text-sm text-gray-600">Double check your information before publishing your Trust Page.</p>
              </div>
              ...
              <div className="p-4 bg-gray-50 rounded-xl border border-gray-200 text-xs space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-[#0A3D62]">{profile.trustScore}/100</span>
                </div>
              </div>
            </div>
          )}
          ...
          <div id="wizard-footer-nav" className="mt-6 pt-4 border-t border-gray-200 flex items-center justify-between">
            <button
              id="btn-wizard-back"
              onClick={handlePrev}
              className="flex items-center space-x-1.5 px-4 py-2 rounded-lg text-xs font-semibold text-gray-600 hover:text-gray-900 hover:bg-gray-100 transition-all"
            >
